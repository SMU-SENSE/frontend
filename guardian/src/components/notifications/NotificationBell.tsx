...
export function NotificationBell() {
  ...
  const queryKey = ['guardian-notifications', user?.id] as const
  const notifications = useQuery({
    queryKey,
    queryFn: () => notificationsApi.list(user!.id, user!.name),
  ...
  return (
    <div className={styles.root} ref={rootRef}>
      <button type="button" className={styles.trigger} aria-label={unreadCount > 0 ? `알림 ${unreadCount}개 확인` : '알림 확인'} aria-expanded={open} aria-haspopup="dialog" onClick={() => setOpen((value) => !value)}>
        <Bell size={20} aria-hidden />
        {unreadCount > 0 ? <span className={styles.badge} aria-hidden>{unreadCount > 9 ? '9+' : unreadCount}</span> : null}
      </button>
      ...
      {open ? (
        <section className={styles.panel} role="dialog" aria-label="보호자 알림">
          ...
          {users.isLoading || notifications.isLoading ? (
            ...
            <div className={styles.state}><span>알림을 불러오지 못했어요</span><button type="button" onClick={() => { users.refetch(); notifications.refetch() }}>다시 시도</button></div>
          ) : !user ? (
